...
public class Alphabet {
 ...
 Picture pic = new Picture(40,40);
 String s = "AACGAACGGTTTACCCCG";
    System.out.println(s.getBytes().length*8);
        int[]  encoded2 = Alphabet.DNA.toIndices(s);
        System.out.println(s.length());
        for(int end: encoded2) {
         //System.out.println(end);  
        	for(int j = 0; j < 40; j++) {
        		for(int k =0; k<40;k++){
        	for(int i = 0; i < DNA.lgR(); i++) {
            boolean bit = ((end >>> (DNA.lgR() - i - 1)) & 1) == 1;

            if(bit) pic.set(j,k,java.awt.Color.black);
            else pic.set(j, k, java.awt.Color.WHITE);
        	}
        		}
        	}
        //BinaryStdOut.write(end,DNA.lgR(),pic);
        }
        pic.show();
